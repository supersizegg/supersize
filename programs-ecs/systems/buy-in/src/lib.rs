...
pub mod buy_in {
    ...
    pub fn execute(ctx: Context<Components>, args: Args) -> Result<Components> {
        ...
        require!(
            ctx.accounts.anteroom.map == ctx.accounts.player.map,
            SupersizeError::MapKeyMismatch
        );
        require!(
            ctx.accounts.player.score == 0.0,
            SupersizeError::AlreadyInGame
        );
        require!(
        ...
        require!(
            buy_in <= ctx.accounts.anteroom.max_buyin,
            SupersizeError::InvalidBuyIn
        );
        require!(
            buy_in >= ctx.accounts.anteroom.min_buyin,
            SupersizeError::InvalidBuyIn
        );
        require!(
            ctx.accounts
        ...
        if **ctx
            .refferal_pda_account()?
            .to_account_info()
            .try_borrow_lamports()?
            == 0
            && (**ctx
                .buddy_member_pda_account()?
                .to_account_info()
                .try_borrow_lamports()?
                > 0)
        {
            ...
            let member_pda_seeds: &[&[u8]] = &[
                b"member_",
                b"supersize",
                b"_",
                member_name_string.as_bytes(),
            ];
            let (derived_member_pda, _bump) =
                Pubkey::find_program_address(member_pda_seeds, &buddy_link_pid);
            require!(
                derived_member_pda == ctx.buddy_member_pda_account()?.key(),
                SupersizeError::InvalidMember
            );
            ...
            */
            ...
            let (derived_referral_pda, refbump) =
                Pubkey::find_program_address(referral_pda_seeds, &enter_pid);
            require!(
                derived_referral_pda == ctx.refferal_pda_account()?.key(),
                SupersizeError::InvalidReferrer
            );

            let referral_token_account_owner_pda_seeds =
                &[b"token_account_owner_pda", token_mint.as_ref()];
            let (referral_derived_token_account_owner_pda, bump) =
                Pubkey::find_program_address(referral_token_account_owner_pda_seeds, &enter_pid);

            let referral_vault_token_account: TokenAccount =
                TokenAccount::try_deserialize_unchecked(
                    &mut (ctx
                        .referral_vault_token_account()?
                        .to_account_info()
                        .data
                        .borrow())
                    .as_ref(),
            ...
            if referral_vault_balance > 0 {
                ...
                let cpi_ctx =
                    CpiContext::new(ctx.token_program()?.to_account_info(), transfer_instruction);
                let scale_factor = 10_u64.pow(decimals);
