...
pub mod movement {
    ...
    pub fn execute(ctx: Context<Components>, args: Args) -> Result<Components> {
        ...
        if player.mass >= 100 {
            ...
            if boosting
                || player.boost_click_time.map_or(false, |boost_click_time| {
                    current_timestamp - boost_click_time < 1
                })
            {
                if boosting {
                ...
                if steps_to_take >= 1 {
                    ...
                    let steps_to_add = if steps_to_take < free_space {
                        steps_to_take
                    } else {
                        free_space
                    };
                    ...
                    for n in 0..steps_to_add {
                        ...
                        let hardvary: u64 = xorshift_output
                            .wrapping_mul(free_space + n + 1)
                            .wrapping_shl(random_shift as u32);
                        ...
                        if clamped_food_x >= section.top_left_x
                            && clamped_food_x < section.top_left_x + 1000
                            && clamped_food_y >= section.top_left_y
                            && clamped_food_y < section.top_left_y + 1000
                        {
                            let encoded_data =
                                encode_food(clamped_food_x, clamped_food_y, step_unit as u16);
